...
<main>
    ...
    <div class="game-info">
        ...
        <!-- ★1 gameStatusが"won"だったら「🎉 勝利！」と表示する -->
        <!-- ヒント：「1」を同じ変数に置き換え、「勝った」を正しい文字列に変更すればOK -->
        {#if gameStatus === "won"}
            <div class="status-message">🎉 勝利！</div>
        <!-- ★2 gameStatusが"lost"だったら「💥 敗北...」と表示する  -->
        <!-- ヒント：「1」を同じ変数に置き換え、「負けた」を正しい文字列に変更すればOK -->
        {:else if gameStatus === "lost"}
            <div class="status-message">💥 敗北...</div>
        {:else}
            <!-- ★3 上記の条件以外には何も表示しない。  -->
            <!-- ヒント：表示したくない文字列はそのまま消すか、コメントアウトすればOK -->
            <!-- 何も表示しない -->
        {/if}
    </div>
